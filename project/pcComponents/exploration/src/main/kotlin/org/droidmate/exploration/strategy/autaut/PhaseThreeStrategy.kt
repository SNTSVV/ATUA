...
import org.droidmate.exploration.modelFeatures.regression.abstractStateElement.AbstractStateManager
import org.droidmate.exploration.modelFeatures.regression.helper.ProbabilityDistribution
...
import org.droidmate.exploration.modelFeatures.regression.staticModel.TransitionPath
import org.droidmate.exploration.modelFeatures.regression.staticModel.WTGNode
...
//                strategyTask = null
    ...
    override fun getPathsToOtherWindows(currentState: State<*>): List<TransitionPath> {
        ...
        do {
            ...
            if (randomOrNot) {
                ...
                destinationAbStates = AbstractStateManager.instance.ABSTRACT_STATES.filter { it.staticNode == randomNode }
            } else {
                targetEvent = phase2TargetEvents.filter { regressionTestingMF.eventWindowCorrelation.containsKey(it.key) }.map { it.key }.random()
                val availableNodes = regressionTestingMF.eventWindowCorrelation[targetEvent!!]!!.filter { node -> AbstractStateManager.instance.ABSTRACT_STATES.any { it.staticNode == node.key } }
                if (availableNodes.isNotEmpty()) {
                    ...
                    destinationAbStates = AbstractStateManager.instance.ABSTRACT_STATES.filter { it.staticNode == targetRandomNode }
                }
            }
        ...
        if (budgetLeft > 0)
        {
            if (phaseState == PhaseState.P3_INITIAL)
            {
                ...
                if(goToAnotherNode.isAvailable(currentState))
                {
                    ...
                    phaseState = PhaseState.P3_GO_TO_ANOTHER_NODE
            ...
            else if (phaseState == PhaseState.P3_GO_TO_ANOTHER_NODE)
            {
            ...
            else if (phaseState == PhaseState.P3_RANDOM_EXPLORATION)
            {
    ...
    fun selectTargetStaticEvents(){
        computeAppStatesScore()
        regressionTestingMF.computeEventWindowCorrelation()
        //pick up randomly a modified method
        val pdForModifiedMethod = ProbabilityDistribution<String>(modifiedMethodWeights)
        val targetModifiedMethod = pdForModifiedMethod.getRandomVariable()
        //get possible triggerring event
        val topCallers = regressionTestingMF.modifiedMethodTopCallersMap[targetModifiedMethod]!!
        val possiblyTriggeringEvents = phase2TargetEvents.map { it.key }.filter {
            topCallers.intersect(it.eventHandlers).isNotEmpty()
        }
        //pick up randomly a static node
        val targetNodes = regressionTestingMF.transitionGraph.edges().filter { possiblyTriggeringEvents.contains(it.label) }.map { it.source.data }
        val targetNode: WTGNode
        if (targetNodes.isNotEmpty())
            targetNode = targetNodes.random()
        else
            targetNode = WTGNode.allMeaningNodes.random()
