...
class Dialog(classType: String,
    ...
    companion object{
        var counter = 0
        val allNodes = ArrayList<Dialog>()
        fun getNodeId(): String = "Dialog-${counter+1}"
        fun getOrCreateNode(nodeId: String, classType: String, fromModel: Boolean=true): Dialog {
            val node = allNodes.find { it.windowId == nodeId }
            if (node != null)
                return node!!
            else
                return Dialog(nodeId = nodeId, classType = classType, fromModel = fromModel)
        }
    }
}