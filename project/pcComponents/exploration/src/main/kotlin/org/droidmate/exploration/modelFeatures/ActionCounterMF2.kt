...
class ActionCounterMF2 : ModelFeature() {
	// we don't want to wait for other modelFeatures (or having them wait for us), therefore create our own (child) job
	override val coroutineContext: CoroutineContext = CoroutineName("ActionCounter2")+Job()
	...
	override suspend fun onNewInteracted(traceId: UUID, targetWidgets: List<Widget>, prevState: State<*>, newState: State<*>): Unit =
			targetWidgets.forEach { target ->
				prevState.uid.let { sId ->
					...
					target.let { wCnt.compute(it.id) { _, m -> m?.incCnt(sId) ?: mutableMapOf(sId to 1) } }
				}
			}
	...
	private val wCnt = ConcurrentHashMap<ConcreteId, MutableMap<UUID, Int>>()
	...
	suspend fun numExplored(s: State<*>): Map<Widget, Int> {
		...
		return s.actionableWidgets.map {
			it to it.id.cntForState(s.uid)//(wCnt[w.uid]?.get(s.uid)?:0)
		}.toMap()
	}
	...
	private fun ConcreteId.cntForState(sId: UUID): Int = wCnt.getCounter(this, sId)
	/** @return how often widget.uid was triggered in the given state-eContext **/
	@Suppress("unused")
	suspend fun widgetCntForState(wId: ConcreteId, sId: UUID): Int {
		this.join()
		return wId.cntForState(sId)
	}

	/** @return how often the widget.uid was triggered other all states **/
	suspend fun widgetCnt(wId: ConcreteId): Int {
		this.join()
