...
class DSTGModelParser (val dstgFolderPath: Path,
    ...
    private fun parseAbstractTransition(abstractTransition: String) {
        ...
        val sourceState = AbstractStateManager.instance.ABSTRACT_STATES.find { it.abstractStateId == UUID.fromString(sourceStateID)  }
        val resState = AbstractStateManager.instance.ABSTRACT_STATES.find { it.abstractStateId == UUID.fromString(resStateID) }
        if (sourceState == null) {
